...
class alicmd:
    ...
    def StorageDB(self, data):
        for i in data:
            self.db_cursor.execute("insert into assets(RegionId ,ZoneId, InstanceId, InstanceName, TagValue, TagKey, PrivateIpAddress, PublicIpAddress, Bandwidth, InstanceType, Cpu, Memory, OSType, OSName, Status, Account, CreationTime, ExpiredTime) VALUES {}".format(i))
            self.mysql_conn.commit()
        #self.mysql_conn.close()

    def StorageDBDisk(self, disk):
        for i in disk:
            self.db_cursor.execute("insert into assets_disk(Account, RegionId, ZoneId, InstanceId, DiskId, Device, Size, Status, CreationTime, ExpiredTime) VALUES {}".format(i))
            self.mysql_conn.commit()
        self.mysql_conn.close()
        

regions = ["cn-hongkong", "ap-southeast-1"]
accounts = [{"AccountName": "",  "AccessKey": "", "AccessKeySecret": "" }]
...
db_cursor.execute("truncate assets;")
db_cursor.execute("truncate assets_disk;")
mysql_conn.commit()
